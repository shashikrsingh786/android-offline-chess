...
io.on('connection', (socket) => {
  ...
  socket.on('disconnect', () => {
    ...
    //if socket is in a room and game still exists
    if (gameId !== '' && gameId && games[gameId]) {
    ...
    // Clean up rooms mapping regardless
    if (rooms[socket.id]) {
      delete rooms[socket.id]
    }
  })
...
server.listen(3001, '0.0.0.0', () => {
  console.log('Server is online on all network interfaces')
})